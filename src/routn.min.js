...
function parseIgnoreHistory(route,keepHistory){if((typeof keepHistory==='boolean')&&!keepHistory){return(routesNotSaved[route]=true);}
return false;}
var useHashForRouting=true,registeredRoutes={},routesNotSaved={};return{setup:function(useHash){useHashForRouting=useHash==undefined?true:useHash;},navigateTo:function(url,data){var context=new routeContext();context.create(url,data);transitionTo(context);},register:function(){var routesIn=(arguments&&arguments[0]&&(arguments[0][0]instanceof Array))?arguments[0]:(Array.prototype.slice.apply(arguments));if(!routesIn||!routesIn.length){console.log("No routes found");return;}
