...
window.onclick=function(e){var destUrl=e.target?e.target.href:null;if(destUrl){var result=getRoutePath(destUrl);if(result){if((isIE()&&result.hasHash)){navigatePropagate(e,destUrl,result.path,true);}else if(!result.hasHash){navigatePropagate(e,destUrl,result.path);}}}}
<<<<<<< HEAD
function getRoutePath(url){if(!url||!url.trim())return null;var path=url.replace(document.location.origin,'');var hashIndex=-1;if(useHashForRouting){hashIndex=path.indexOf('#');if(hashIndex!==-1){path=path.substring(hashIndex+1);}}
=======
function getRoutePath(url){if(isEmpty(url))return null;var path=url.replace(document.location.origin,'');var hashIndex=-1;if(useHashForRouting){hashIndex=path.indexOf('#');if(hashIndex!==-1){path=path.substring(hashIndex+1);}}
>>>>>>> master
var hasHash=hashIndex!==-1;if(!hasHash){var queryIndex=path.indexOf('?');if(queryIndex!==-1){path=path.substring(0,queryIndex);}}
...
if(registeredRoutes[newRoute]){console.log(newRoute," route is already registerd");return false;}
return true;}
function getUrlParts(url){if(!url)return null;var segments=url.split('/').filter(function(s){return!isEmpty(s)});var len=segments.length;var parts=[];for(var i=0;i<len;i++){var item=segments[i].trim();var part=new routePart();part.name=item.replace(':','').toLowerCase();var isId=!(item.indexOf(':')===-1&&isNaN(item));part.type=isId?'id':'path';parts.push(part);}
>>>>>>> master
return parts;}
...
function transition(context){var route=parseRoute(context.path);if(route){context.params=route.params;executeHandler(route.handlers,context,0);}}
...
function navigateAway(url){document.location.href=url;return this;}
function navigateTo(url,data){var path=getRoutePath(url).path;transitionTo(url,path,data);return this;}
=======
...
return this;}
<<<<<<< HEAD
var useHashForRouting=true,registeredRoutes={},routesWithoutHistory={};return new function(){var _this=this,dummy,historySupported=!!(window.history&&window.history.pushState&&window.history.replaceState&&window.onpopstate);if(!historySupported){dummy=function(){return _this};console.log("routn works only with browsers that support HTML5 history APIs");}
this.setup=historySupported?setup.bind(this):dummy;this.navigateAway=historySupported?navigateAway.bind(this):dummy;this.navigateTo=historySupported?navigateTo.bind(this):dummy;this.register=historySupported?register.bind(this):dummy;};})();
=======
var useHashForRouting=true,registeredRoutes={},routesWithoutHistory={};return new function(){if(!(window.history&&window.history.pushState&&window.history.replaceState&&window.onpopstate)){var _this=this;this.setup=this.navigateAway=this.navigateTo=this.register=function(){return _this};console.log("routn works only with browsers that support HTML5 history APIs");}else{this.setup=setup.bind(this);this.navigateAway=navigateAway.bind(this);this.navigateTo=navigateTo.bind(this);this.register=register.bind(this);}};})();
>>>>>>> master
